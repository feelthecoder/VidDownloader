...
public class PregMatchUtils {
     ...
     */
    public PregMatchUtils() {
    }

    public static int preg_match(String pattern, String subject) {
        Pattern p = PregMatchUtils.compile(pattern);
    ...
    public static String[] preg_match_all(String pattern, String subject, int groups) {
        Pattern p = PregMatchUtils.compile(PregMatchUtils.getPatternWithoutFlags(pattern));
    ...
    public static String preg_replace(String[] patterns, String[] replacements, String subject) {
        for (int i = 0; i < patterns.length; i++) {
            subject = PregMatchUtils.preg_replace(patterns[i], (replacements[i] != null) ? replacements[i] : "", subject);
        }
    ...
    public static String preg_replace(String[] patterns, String[] replacements, String subject, String[] indexof) {
        ...
        for (int i = 0; i < patterns.length; i++) {
            if (subject.indexOf(indexof[i]) != -1) {
                subject = PregMatchUtils.preg_replace(patterns[i], replacements[i], subject);
            }
        }
    ...
    public static String preg_replace(String[] patterns, String replacement, String subject) {
        for (int i = 0; i < patterns.length; i++) {
            subject = PregMatchUtils.preg_replace(patterns[i], replacement, subject);
        }
  ...
  Pattern p = PregMatchUtils.compile(regex);
  Matcher m = p.matcher(text); // get a matcher object
  if(m.find()) {
  //return m.group();
  Pattern p2 = PregMatchUtils.compile(pattern);
  ...
  return "";
    ...
    public static String preg_quote(String str) {
        return PregMatchUtils.str_replace(PregMatchUtils.SLASH_CHARACTERS, PregMatchUtils.SLASH_CHARACTERS_SUBSTITUTION, str);
    }
    ...
    public static Pattern compile(String pattern) {
        //System.out.println("compile(pattern:" + pattern + ") => woFlags:'" + PregMatchUtils.getPatternWithoutFlags(pattern) + "', flags: " + PregMatchUtils.getPatternFlags(pattern));
        return Pattern.compile(PregMatchUtils.getPatternWithoutFlags(pattern), PregMatchUtils.getPatternFlags(pattern));
    }
